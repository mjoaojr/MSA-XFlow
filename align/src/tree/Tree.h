...
class Tree
{
    public:
        ...
        int readTree(Alignment* alignPtr, const string& treeFileName, int firstSeq, 
                     int lastSeq);
        unique_ptr<AlignmentSteps> createSets(int firstSeq, int lastSeq);
    ...
    private:
        ...
        void orderNodes();
        int calcWeightUPGMA(int leaf);
        int calcWeightNJ(int leaf);
/*
	void calcWeights(vector<int>* seqWeights);
	void doWeightCalc(float weightSoFar, vector<float>* weights, TreeNode* t);
*/
        ...
        void setInfo(TreeNode* p, TreeNode* parent, int pleaf, string pname, float
                     pdist);
	void printPreOrderRec(TreeNode *p);
