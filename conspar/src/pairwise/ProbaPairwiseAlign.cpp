...
{
	vector<float> s(strlen(alphabetDefault), 0);
	int l,a,b, i, j;
	const char *alphabet;
	float		**emitPairs,
			**transMat;
	vector<float>	emitSingle(256, 0);
...
*/
	...
	l=strlen (alphabet);
//	fill_n(back_inserter(emitSingle), 256, 0);
...
*/
	...
	fill_n(back_inserter(initialDistribution), 2*NumMatrixTypes+1, 0);
//	initialDistribution.resize(2*NumMatrixTypes+1);
	ProbabilisticModel (NumMatrixTypes,NumInsertStates,initDistrib2Default, emitSingle,emitPairs,gapOpen2Default,gapExtend2Default, transMat, initialDistribution);
...
void ProbaPairwiseAlign::pairwiseAlign(Alignment *alignPtr, DistMatrix *distMat, int iStart, int iEnd, int jStart, int jEnd)
{
	...
	try
	{
		...
		ini = omp_get_wtime();
		#pragma omp parallel for schedule (dynamic, 1) private (si, sj, probaScore)
//		#pragma omp parallel for schedule (static, 1) private (si, sj, probaScore)
//		#pragma omp parallel for schedule (static) private (si, sj, probaScore)
//		#pragma omp parallel for schedule (guided, 1) private (si, sj, probaScore)
		//for (si = utilityObject->MAX(0, iStart); si < _numSeqs && si < iEnd; si++)
		for (si = utilityObject->MAX(0, iStart); si < _numSeqs; si++)
		{
			const vector<int>* ptrToSeq1;
			const vector<int>* ptrToSeq2;

		    int lenSeq1 = alignPtr->getSeqLength(si + 1);

		    for (sj = utilityObject->MAX(si+1, jStart+1); sj < _numSeqs && sj < jEnd; sj++)
		    {
			int lenSeq2 = alignPtr->getSeqLength(sj + 1);
			// align the sequences
		    
			ptrToSeq1 = alignPtr->getSequence(si + 1);
			ptrToSeq2 = alignPtr->getSequence(sj + 1);
		    
			// Use Proba Pair to calculate score
			probaScore = proba_pair_wise(si, sj, lenSeq1, lenSeq2, ptrToSeq1, ptrToSeq2);
			#pragma omp critical
			{
...
int ProbaPairwiseAlign::ProbabilisticModel (int NumMatrixTypes, int NumInsertStates, const float *initDistribMat, vector<float> emitSingle, float **emitPairs, const float *gapOpen, const float *gapExtend, float **transMat, vector<float> &initialDistribution)
{
...
int ProbaPairwiseAlign::get_tot_prob2 (int nstates, float *TmatchProb, float ***TinsProb, int lenSeq1, int lenSeq2, const vector<int>* ptrToSeq1, const vector<int>* ptrToSeq2)
{
...
//			TinsProb[0][k][i] = (float)insProb[asclu[getElem1(i-1)]][k];
			TinsProb[0][k][i] = (float)insProb[asclu[(*ptrToSeq1)[i]]][k];
		}
//              printf ("\n");
	}
...
//			TinsProb[1][k][j] = (float)insProb[asclu[getElem2(j-1)]][k];
			TinsProb[1][k][j] = (float)insProb[asclu[(*ptrToSeq2)[j]]][k];
		}
	}
...
//					if ((lu[convlu[getElem1(i-1)]] == lu[r1]) && (lu[convlu[getElem2(j-1)]] == lu[r2]))
					if ((lu[convlu[(*ptrToSeq1)[i]]] == lu[r1]) && (lu[convlu[(*ptrToSeq2)[j]]] == lu[r2]))
					{
...
//			TmatchProb[ij]+=matchProb[asclu[getElem1(i-1)]][asclu[getElem2(j-1)]];
			TmatchProb[ij]+=matchProb[asclu[(*ptrToSeq1)[i]]][asclu[(*ptrToSeq2)[j]]];
		}
	}
...
float * ProbaPairwiseAlign::forward_proba_pair_wise (int NumMatrixTypes, int NumInsertStates, vector<float> initialDistribution,float *matchProb, float ***insProb, int lenSeq1, int lenSeq2)
{
	float *forward;
	int max_l;
...
float *ProbaPairwiseAlign::backward_proba_pair_wise (int NumMatrixTypes, int NumInsertStates, vector<float> initialDistribution,float *matchProb, float ***insProb, int lenSeq1, int lenSeq2)
{
	float *backward;
	int max_l;
...
float ProbaPairwiseAlign::ProbaMatrix2CL (int NumMatrixTypes, int NumInsertStates, float *forward, float *backward, float thr, int s1, int s2, vector<float> &posterior, int lenSeq1, int lenSeq2)
{
	float totalProb,
		maxTotProb;
	int ij, i, j,k;
...
float ProbaPairwiseAlign::proba_pair_wise (int s1, int s2, int lenSeq1, int lenSeq2, const vector<int>* ptrToSeq1, const vector<int>* ptrToSeq2)
{
	float ***TinsProb;
	float *TmatchProb;
	float	score;
	...
	TmatchProb = (float *)malloc (l * sizeof (float));

	l=max(lenSeq1,lenSeq2)+1;

	TinsProb = declare_array3 (2, NumMatrixTypes, l, sizeof (float));

	get_tot_prob2 (NumMatrixTypes, TmatchProb, TinsProb, lenSeq1, lenSeq2, ptrToSeq1, ptrToSeq2);

	F=forward_proba_pair_wise (NumMatrixTypes,NumInsertStates, initialDistribution,TmatchProb,TinsProb, lenSeq1, lenSeq2);
	B=backward_proba_pair_wise (NumMatrixTypes,NumInsertStates, initialDistribution,TmatchProb,TinsProb, lenSeq1, lenSeq2);
	score = ProbaMatrix2CL(NumMatrixTypes,NumInsertStates, F, B, thr, s1, s2, posterior, lenSeq1, lenSeq2);
