...
*/
		/* com hash
		extEdges = alnPtr->getPCL()->extEdges;
		indExtEdges = alnPtr->getPCL()->indExtEdges;
		*/
		umap = alnPtr->getPCL()->umap;
/*
	}
*/
...
*/
		...
		{
/*
			if ((*indExtEdges)[m-1] != -1)
			{
				for (i = (*indExtEdges)[m-1]; i < extEdges->size() && (m == (*extEdges)[i].p1); i++)
					if ((*extEdges)[i].p2 == n)
						break;

				if (i < extEdges->size() && ((*extEdges)[i].p1 == m))
				{
//					printf ("[%d, %d, %d, %d]\n", m, n, (int)((*extEdges)[i].score*100), score);
					score += (int)((*extEdges)[i].score*100);
				}
			}
*/
			if (umap->count(KEY(m,n)))
			{
//				sc = (int)(umap->at(KEY(m,n))*100);
//				printf ("[%d, %d, %d, %d]\n", m, n, sc, score);
				score += (int)(umap->at(KEY(m,n))*100);
			}
		}
		else
		{
/*
			if ((*indExtEdges)[n-1] != -1)
			{
				for (i = (*indExtEdges)[n-1]; i < extEdges->size() && (n == (*extEdges)[i].p1); i++)
					if ((*extEdges)[i].p2 == m)
						break;

				if (i < extEdges->size() && ((*extEdges)[i].p1 == n))
				{
//					printf ("[%d, %d, %d, %d]\n", m, n, (int)((*extEdges)[i].score*100), score);
					score += (int)((*extEdges)[i].score*100);
				}
...
*/
			if (umap->count(KEY(n,m)))
			{
//				sc = (int)(umap->at(KEY(n,m))*100);
//				printf ("[%d, %d, %d, %d]\n", m, n, sc, score);
				score += (int)(umap->at(KEY(n,m))*100);
			}
		}
/*
	}
