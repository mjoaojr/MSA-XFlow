...
class Alignment
{
    public:
        ...
        void addSeqWeight(const vector<int>* _seqWeight);
        ...
        void setConstraints(Constraints *constraintsList){pCL = constraintsList;};         
        ...
        //Constraints *getConstraints(){return pCL;};         
	...
	/*
	 * To help Muscle convertion
	 */
	unsigned long *getSequenceIds(int index){return &(sequenceIds[index]);}
	vector<string> *getSequenceNames(){return &names;}
	unsigned long getSeqByName(string name);
	

	/*
	 * Consistency methods
	 */
	Constraints *getPCL(){return pCL;}
	void setPCL(Constraints *constraintsList){pCL = constraintsList;}
/*
	void setSparseMatrix(int a, int b, int lenA, int lenB, vector<float> post){pCL->setSparseMatrix(a, b, lenA, lenB, post);}
	vector<vector<SparseMatrix *>>getSparseMatrices (){return pCL->getSparseMatrices();}
	 */
