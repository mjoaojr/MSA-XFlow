			...
			for (a = 0; a < len1; a++)
			{
				...
				for (x = 0; x < len2; x++)
				{
					...
					else 
					{
						if (hasch[t_s2][t_r2])
						{
							score += min(((float)hasch[t_s2][t_r2]),((float)(*pEdges)[s2][r2][x].getWeight()));
						}
					}
					norm2++;
				}
...
//	printf ("\nnEdges = %d e finalEdges = %d\n", nEdges, finalEdges);
	 ...
	 */
	finalEdges = nEdges;
	for (s1 = 0, a = 0; s1 < _numSeqs; s1++)
	{
		for (r1 = 0; r1 <= alignPtr->getSeqLength (s1 + 1) && r1 < (*pEdges)[s1].size(); r1++)
		{
			len1 = (*pEdges)[s1][r1].size();
			for (x = 0; x < len1;)
			{
...
//				printf ("%d %d %d %d %d\n", s1, r1, t_s1, t_r1, scores[a]);
				if (scores[a] > 10)
				{
					(*pEdges)[s1][r1][x].setWeight(scores[a++]);
					x++;
				}
				else
				{
...
*/
	...
	/*
	 * MJ
	 * Tenho que liberar o extEdges anterior
	 * O mesmo para o indExtEdges
	 */
	if (pCL->extEdges != NULL)
		delete pCL->extEdges;
	if (pCL->umap != NULL)
		delete pCL->umap;
...
//		printf ("\n Seq %d: ", (int)alignPtr->getUniqueId(ls[0][a]+1) - 1);
		for (int g = 1, r = 1; g < ptrSeq->size(); g++)
		{
			int b;
			b = ptrSeq->at(g);
//			printf ("%02d ", b);
...
//	extEdges = new vector<ExtEdge>;
	umap = new std::unordered_map<unsigned long int, float>;
	// Preciso disso?
...
//			s = ls[0][si];
			...
			{
//				printf ("[%d, %d, %d]\n", (*pEdges)[1][2][0].getS2(), (*pEdges)[1][2][0].getS2(), (*pEdges)[1][2][0].getS2());
...
// com hash
//					extEdges->push_back ({p1, p2, score});
					#pragma omp critical
					{
					(*umap)[KEY(p1, p2)] = score;
					}
				}
				catch (exception& ex)
				{
...
/*
	printf ("ExtEdges size: %lu - l1 size:%d\n", extEdges->size(), l1);
...
/*
	...
	if (pCL->indExtEdges != NULL)
		delete pCL->indExtEdges;

	if (pCL->extEdges->size() > 0)
	{
