...
#include "pairwise/LcsPairwiseAlign.h"
#include "pairwise/KmersPairwiseAlign.h"
#include "pairwise/ProbaPairwiseAlign.h"
#include "multipleAlign/LowScoreSegProfile.h"
#include "multipleAlign/Iteration.h"
#include "iterative/MuscleIteration.h"
...
/*
    ...
    {
	if (userParameters->getLcsPairAlign())
	{
		pairwiseDist = new LcsPairwiseAlign();
	}
	else
	{
		if (userParameters->getKmersPairAlign())
		{
			pairwiseDist = new KmersPairwiseAlign();
		}
		else
			if (userParameters->getProbaPairAlign())
			{
				pairwiseDist = new ProbaPairwiseAlign();
			}
			else
			{
				pairwiseDist = new FullPairwiseAlign();
...
*/
	...
	/*
	 * Need to set pairwiseConsist to PROBA when Consistency is used.
	 * But if PROBA is the similarity method, it is not necessary to call it again.
	PairwiseAlignBase* pairwiseConsist;
	// Generating Consistency
	if (userParameters->getConsistency())
	{
		if (userParameters->getProbaPairAlign())
		{
			pairwiseConsist = pairwiseDist;
		}
		else
		{
			pairwiseConsist = new ProbaPairwiseAlign();
			pairwiseConsist->pairwiseAlign(&alignmentObj, &distMat, 0, _numSeqs, 0, _numSeqs);
			pCL = alignmentObj.getConstraints();
		}
	}
	else
	{
		pCL = NULL;
		pairwiseConsist = NULL;
	}
	 */
...
/*
	...
	cout << '\n';
	distMat.writeToFile(path+"full.ss");
*/

/*
	if (userParameters->getConsistency() && pairwiseConsist == pairwiseDist)
		pCL = alignmentObj.getConstraints();

	alignmentObj.setConstraints(pCL);

	delete pairwiseDist;
	if (userParameters->getConsistency() && pairwiseConsist != pairwiseDist)
		delete pairwiseConsist;
*/

	Constraints     *pCL = new Constraints(_numSeqs);
	pCL->loadEdges (userParameters->getCLName());
//	pCL->printEdges();
	alignmentObj.setConstraints(pCL);

	bool success = false;
	unique_ptr<AlignmentSteps> progSteps;
	...
	distMat.readFromFile (userParameters->getSSName());
//	distMat.printArray ();
//	cout << *phylipName << " - eca\n";
	TreeInterface calcSteps;
	progSteps = calcSteps.getWeightsAndStepsFromTree (&alignmentObj, &distMat, phylipName, &seqWeight, 1, _numSeqs, &success);
//	distMat.printArray ();
...
/*
    ...
    if(userParameters->getDoRemoveFirstIteration() == MUSCLE)
    {
        MuscleIteration iterateObj;
        iterateObj.restrictedTreePartition (&alignmentObj, progSteps.get(), phylipName);
    }
...
/*
   ...
   //
    ...
    // Generate distance matrix!
	Constraints     *pCL;
	if (pairwiseConsist != NULL && pairwiseConsist != pairwiseDist)
	{
		pairwiseConsist->pairwiseAlign(&alignmentObj, &distMat, 0, _numSeqs, 0, _numSeqs);
		pCL = alignmentObj.getConstraints();
	}
