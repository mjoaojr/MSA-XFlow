    ...
    setOutputPim(-1)
{
    ...
    try
    {
        ...
        clusterAlgorithm = new StringArray(5);
        iterationArg = new StringArray(5);
    ...
    (*clusterAlgorithm)[2] = "slmin";
    (*clusterAlgorithm)[3] = "slmax";
    (*clusterAlgorithm)[4] = "";

    (*iterationArg)[0] = "tree";
    (*iterationArg)[1] = "alignment";
    (*iterationArg)[2] = "none";
    (*iterationArg)[3] = "muscle";
    (*iterationArg)[4] = "";
    ...
    cmdLineFile[ctr++] = getCmdLineDataStruct("ssfile", &setSSfile, FILARG, NULL);
    cmdLineFile[ctr++] = getCmdLineDataStruct("clfile", &setCLfile, FILARG, NULL);
    ...
    cmdLineVerb[ctr++] = getCmdLineDataStruct("lcs", &setLcs, NOARG, NULL);
    cmdLineVerb[ctr++] = getCmdLineDataStruct("kmers", &setKmers, NOARG, NULL);
    cmdLineVerb[ctr++] = getCmdLineDataStruct("proba", &setProba, NOARG, NULL);
    cmdLineVerb[ctr++] = getCmdLineDataStruct("convert", &setConvert, NOARG, NULL);
    cmdLineVerb[ctr++] = getCmdLineDataStruct("consistency", &setConsistency, NOARG, NULL);
    ...
    cmdLineVerb[ctr++] = getCmdLineDataStruct("iteration", &setDoIteration, OPTARG, iterationArg);
    ...
    cmdLinePara[ctr++] = getCmdLineDataStruct("nthr", &setNthr, NOARG, NULL);
...
//**********************************************************************
    ...
    if (setNthr != -1)
    {
            ...
            if(logObject && DEBUGLOG)
            {
                logObject->logMsg("    Setting max allowed OpenMP threads.");
            }
        #endif
        temp = 0;
        if((*paramArg)[setNthr].length() > 0)
        {
            if (sscanf((*paramArg)[setNthr].c_str(),"%d", &temp) != 1)
            {
                reportBadOptionAndExit("nthr", "integer");
            }
        }
        if(temp > 0)
        {
            userParameters->setNthr(temp);
        }
        else
        {
            cerr << "Cannot use a negative value for number of threads. Using 1"  << std::endl;
        }
    ...
    if(setDoIteration != -1)
    {
        ...
        { 
            temp = findMatch((*paramArg)[setDoIteration], iterationArg, 4);
            if(temp == 0)
            {
            ...
            {               
                userParameters->setDoRemoveFirstIteration(MUSCLE);
            }
            else if(temp == 2)
            {               
...
//*************************************************
    ...
    if(setSSfile != -1) 
    {
        if((*paramArg)[setSSfile].length() <= 0) 
        {
            exitWithErrorMsg("Bad similarity score file name");
        }
        userParameters->setSSName((*paramArg)[setSSfile]);

        #if DEBUGFULL 
            if(logObject && DEBUGLOG)
            {
                string msg = "Similarity score file name has been set to ";
                msg += userParameters->getSSName();             
                logObject->logMsg(msg);
            }
        #endif        
    }
    ...
    if(setCLfile != -1) 
    {
        if((*paramArg)[setCLfile].length() <= 0) 
        {
            exitWithErrorMsg("Bad constraints list file name");
        }
        userParameters->setCLName((*paramArg)[setCLfile]);

        #if DEBUGFULL 
            if(logObject && DEBUGLOG)
            {
                string msg = "Constraints list file name has been set to ";
                msg += userParameters->getCLName();             
                logObject->logMsg(msg);
            }
        #endif        
    }
...
//** ? /quicktree 
    ...
    if(setLcs != -1)
    {
        userParameters->setLcsPairAlign(true);
    }

    if(setKmers != -1)
    {
        userParameters->setKmersPairAlign(true);
    }

    if(setProba != -1)
    {
        userParameters->setProbaPairAlign(true);
    }

    if(setConsistency != -1)
    {
        userParameters->setConsistency(true);
    }
...
void CommandLineParser::setOptionalParam()
{
    ...
    if(setTreeAlgorithm != -1)
    {
        ...
        { 
		temp = findMatch((*paramArg)[setTreeAlgorithm], clusterAlgorithm, 4);
		switch (temp)
		{
		    case 0:
			...
			break;
		    case 2:
			userParameters->setClusterAlgorithm(SLMIN);
			break;
		    case 3:
			userParameters->setClusterAlgorithm(SLMAX);
			break;
		    default:
