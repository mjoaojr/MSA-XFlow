...
process full 
{
	input:
		path(in)
	output:
		tuple val (in.baseName), path ("${in.baseName}.in_tfa"), path ("${in.baseName}.${mode}.ss"), val (mode)
...
//	cpus = Runtime.runtime.availableProcessors() - params.n
	...
	input:
		path(in)
	output:
		tuple val (in.baseName), path ("${in.baseName}.in_tfa"), path ("${in.baseName}.${mode}.ss"), val (mode), path ("${in.baseName}.cl")
