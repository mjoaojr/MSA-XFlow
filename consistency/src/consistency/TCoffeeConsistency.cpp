...
//printf ("%lu\n", _numSeqs*(maxLen+1)*sizeof(int));
	

//	finalEdges = 0;
	/*
	 * Locais ao loop externo
	 * s1, r1, norm1, len1, x, t_s1, t_r1
	 *
	 * hasch - colocar um para cada s1.
	 * scores - transformar numa matriz de 3 dimensões
	 */
	for (s1 = 0; s1 < _numSeqs; s1++)
	{
//		for (r1 = 1; r1 <= alignPtr->getSeqLength (s1 + 1) && r1 < (*pEdges)[s1].size(); r1++)
		for (r1 = 1; r1 < (*pEdges)[s1].size(); r1++)
		{
			norm1=0;
...
//	printf ("O tamanho de scores é: %ld\n", scores.size());
