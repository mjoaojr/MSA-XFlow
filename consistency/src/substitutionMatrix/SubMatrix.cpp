 ...
 * @param name 
 */
void SubMatrix::printInFormat(vector<short>& temp, char const* name)
{
    char nameOfFile[30];
 ...
 * @param name 
 */
void SubMatrix::printVectorToFile(vector<short>& temp, char const* name)
{
    char nameOfFile[30];
    strcpy(nameOfFile, name);
    strcat(nameOfFile, ".out");
...
int SubMatrix::getAlnScoreMatrix(int matrix[NUMRES][NUMRES])
{
    ...
    */
    // 1.83 style
	// MJ
    // Original _maxNumRes = getMatrix(blosum45mtVec, &defaultAAXref, matrix, true, 100);
    _maxNumRes = getMatrix(pam120mtVec, &defaultAAXref, matrix, true, 100);
    //_maxNumRes = getMatrix(gon250mtVec, &defaultAAXref, matrix, true, 10);
