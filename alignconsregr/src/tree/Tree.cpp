 ...
 * @return 
 */
unique_ptr<AlignmentSteps> Tree::createSets(int firstSeq, int lastSeq)
{
    unique_ptr<AlignmentSteps> progAlignSteps;
 ...
 * @return 
 */
int Tree::calcWeightUPGMA(int leaf)
{
    clustalw::TreeNode* p;
    float weight = 0.0;
 ...
 * @return 
 */
int Tree::calcWeightNJ(int leaf)
{
    clustalw::TreeNode* p;
    float weight = 0.0;
...
/*
void Tree::calcWeights(vector<int>* seqWeights)
{
...
void Tree::doWeightCalc(float weightSoFar, vector<float>* weights, TreeNode* t)
{
    if(t != NULL)
    {
...
void Tree::printPreOrderRec(TreeNode *p)
{
	if (p == NULL)
		return;
	...
	if (p->leaf)
	{
		printf ("p: %p\tparent: %p\tleft: %p\tright: %p\torder: %d\tdist: %f\tname: %s\n", p, p->parent, p->left, p->right, p->order, p->dist, p->name.c_str());
		return;
	}	

	printf ("p: %p\tparent: %p\tleft: %p\tright: %p\torder: %d\tdist: %f\tname: %s\n", p, p->parent, p->left, p->right, p->order, p->dist, p->name.c_str());
	printPreOrderRec (p->left);
	printPreOrderRec (p->right);
}
