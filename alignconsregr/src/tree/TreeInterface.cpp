                                          ...
                                          int numSeqs, bool* success)         
{
	/**
	  ...
	  */
	if (userParameters->getClusterAlgorithm() != NJ)
	    return getWeightsAndStepsFromTreeUPGMA(alignPtr, distMat, treeName, seqWeights, 
...
/**
 * The function readTreeAndCalcWeights is used to read in the tree given the treeName.
 * It then calls the appropriate functions to calc the seqWeights and make sure the matrix 
 * is in similarity mode. 
 */                                                                                
int
TreeInterface::readTreeAndCalcWeights(Tree* groupTree, Alignment* alignPtr, 
                                        DistMatrix* distMat, string* treeName,
                                        vector<int>* seqWeights, int fSeq, int nSeqs)
{
    int status = 0;
    ...
    for (int i = 0; i < nSeqs; i++)
    {
        ...
        for (int j = 0; j < i; j++)
        {
            value = 100.0 - (*distMat)(i + 1, j + 1) * 100.0;
...
unique_ptr<AlignmentSteps>
TreeInterface::getWeightsAndStepsFromTreeUPGMA(Alignment* alignPtr, 
                                            DistMatrix* distMat, string* treeName,
                                            vector<int>* seqWeights, int fSeq, int numSeqs, 
                                            bool* success)
{
    ...
    int status;
    status = readTreeAndCalcWeights(&groupTree, alignPtr, distMat, treeName, 
                                      seqWeights, fSeq, numSeqs);
...
unique_ptr<AlignmentSteps> TreeInterface::generateTreeFromDistMatUPGMA(RootedGuideTree* guideTree, DistMatrix* distMat, Alignment *alignPtr, int seq1, int nSeqs, 
                                            string* phylipName, bool* success)
{
    unique_ptr<AlignmentSteps> progSteps;
    string copyOfPhylipName = string(*phylipName);     
