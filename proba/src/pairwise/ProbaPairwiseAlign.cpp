...
/* Maneira burra 
	...
	for (i=1; i<=lenSeq1; i++)
	{
		...
		for (k=0; k<nstates; k++)
		{
			...
			{
				*
				 * Se o caracter relativo ao r for o mesmo da posição i-1 da seq, TinsProb = insProb, senão é 0.
				 *
				if (lu[convlu[getElem1(i-1)]] == lu[r])
				{
					TinsProb[0][k][i]+=(float)insProb[lu[r]][k];
					printf ("Somei %f\n", (float)insProb[lu[r]][k]);
				}
			}
			printf ("TinsProb = %f\n", TinsProb[0][k][i]);
		}
	...
	for (j=1; j<=lenSeq2; j++)
	{
		for (k=0; k<nstates; k++)
		{
			...
			for (r=0; r<20; r++)
			{
				*
				 * Se o caracter relativo ao r for o mesmo da posição i-1 da seq, TinsProb = insProb, senão é 0.
				TinsProb[1][k][j]+=(float)prf1[j-1][r]*insProb[lu[r]][k];
				 *
				if (lu[convlu[getElem2(j-1)]] == lu[r])
					TinsProb[1][k][j]+=(float)insProb[lu[r]][k];
			}
		}
	}
*/
