...
class SparseMatrix {
  ...
  vector<int> rowSize;                                     // rowSize[i] = # of cells in row i
  vector<PIF> data;                           // data values
  vector<vector<PIF>::iterator> rowPtrs;  // pointers to the beginning of each row
 ...
 public:
  ...
  SparseMatrix (int seq1Length, int seq2Length, const vector<float> &posterior) :
  ...
  vector<float> *GetPosterior () const {
    ...
    for (int i = 1; i <= seq1Length; i++){
      vector<float>::iterator postPtr = posterior.begin() + i * (seq2Length+1);
      for (int j = 0; j < rowSize[i]; j++){
