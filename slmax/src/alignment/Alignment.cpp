...
void Alignment::pasteSequencesIntoPosition(vector<Sequence>* seqVector, int pos, bool explicitPasteToProfile2)
{
...
void Alignment::addSeqWeight(const vector<int>* _seqWeight)
{
...
vector<Sequence> Alignment::cutSelectedSequencesFromAlignment(vector<int>* selected)
{
    ...
    for(int i = 1; i < sizeOfSelected && i <= intialNumSeqs; i++)
    {
        if((*selected)[i] == 1)
        {
            ...
            Sequence SeqToCut(&seqArray[i - numCutSoFar], *namesIterator, *titlesIterator, *sequenceIdsIterator);
...
unsigned long Alignment::getSeqByName(string name)
{
	for(int i = 1; i <= numSeqs; i++)
	{
//		printf ("%lu: %s", sequenceIds[i], names[i].c_str());

		if (name == names[i])
			return sequenceIds[i];
	}
